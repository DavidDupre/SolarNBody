	...
	public void draw(double centerDistance) {
		...
		if (parent != null && drawConic) {
			...
			HashMap<String, Double> orb = Astrophysics.toOrbitalElements(r, v,
					parent);
			double anomaly = orb.get("v"); // get the current true anomaly so
											// the ellipse passes through the
											// body and looks good
		...
		// Draw a transparent sphere proportional to the zoom level (stays the
		// same size on screen)
		if (drawBigSphere) {
			GL11.glColor4f(color[0], color[1], color[2], .5f);
			image.draw((float) (100000 * centerDistance), 16, 16);
		}
		GL11.glBegin(GL11.GL_POINTS); // This is vital to making this entire
										// file work. Don't ask why.
